...
class PlaybackStorage {
  ...
  static PlaybackState? load() {
    ...
    try {
      ...
      final index =
          int.tryParse(storage.getItem(_keyIndex) ?? '') ?? 0;
      final progress =
      ...
      final contextLabel =
          storage.getItem(_keyContext) ?? 'Now Playing';
      return PlaybackState(
        queue: queue,
        index: index.clamp(0, queue.length - 1),
        progress: progress.clamp(0.0, 1.0),
        mode: PlaybackMode.values.where((m) => m.name == modeName).firstOrNull ??
            PlaybackMode.audio,
  ...
  static void clear() {
    ...
    if (storage == null) return;
    for (final key in [
      _keyQueue, _keyIndex, _keyProgress,
      _keyMode, _keyOrderMode, _keyContext,
    ]) {
